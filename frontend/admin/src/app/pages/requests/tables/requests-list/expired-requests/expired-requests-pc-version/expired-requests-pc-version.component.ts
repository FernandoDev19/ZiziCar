...
import { DataTableForRequestAdminInterface, DataTableForRequestProviderInterface } from '../../../../interfaces/data-table-for-requests.interface';
...
@Component({
  selector: 'app-expired-requests-pc-version',
  standalone: true,
  imports: [CommonModule, FontAwesomeModule, TableModule],
  templateUrl: './expired-requests-pc-version.component.html',
  styleUrl: './expired-requests-pc-version.component.css'
})
export class ExpiredRequestsPcVersionComponent {
  @Input() requestsAdmin!: DataTableForRequestAdminInterface[];
  ...
  hasQuoteOrAnswer(requestId: string): QuoteOrAnswerStatus {
    const request = this.requestsAdmin.find((entry) => entry.id === requestId)
