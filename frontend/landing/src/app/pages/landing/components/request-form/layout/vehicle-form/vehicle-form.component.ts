...
import { GetTransmissionInterface } from '../../../../../../common/interfaces/transmission.interface';
...
export class VehicleFormComponent implements OnInit, AfterViewInit {
  ...
  selectOptionOfPopover(type: string, option: any) {
    ...
    if (type == 'entryCity' || type == 'devolutionCity') {
      cities = this.countries.flatMap((country) => country.cities);
    }

    if (type == 'entryDate' || type == 'devolutionDate') {
      dateFormatted = new Date(option).toLocaleString('es-ES', {
    ...
    switch (type) {
      case 'gamma':
        ...
        this.gammaSelected = {
          ...
          average_price:
            this.getAveragePriceForGamma(getGammaSelectedNow?.id || '') || 0,
        };
      ...
      case 'transmission':
        ...
        if (
          getTransmissionSelectedNow?.name == this.transmissionSelected.name
        ) {
        ...
        this.vehicleForm
          .get('transmission_id')
          ?.setValue(getTransmissionSelectedNow?.id);
        this.transmissionSelected = {
          name: getTransmissionSelectedNow?.name || 'Error',
          image: getTransmissionSelectedNow?.image_url || '',
        };
        this.popoverTransmission.hide();
      ...
      case 'entryCity':
        const entryCitySelectedNow = cities.find(
          (city) => city.city_id === option
        );
        ...
        this.vehicleForm
          .get('id_entry_city')
          ?.setValue(entryCitySelectedNow?.city_id);

        if (entryCitySelectedNow?.city_name != this.devolutionCitySelected) {
          this.devolutionCitySelected =
            entryCitySelectedNow?.city_name || 'Error';
          this.vehicleForm
            .get('id_devolution_city')
            ?.setValue(entryCitySelectedNow?.city_id);
        }
      ...
      case 'devolutionCity':
        const devolutionCitySelectedNow = cities.find(
          (city) => city.city_id === option
        );

        if (devolutionCitySelectedNow?.city_name != this.entryCitySelected) {
          this.communicationService.message.emit(
            'Para devoluciones en ciudades distintas a la recogida, aplican recargos. Estos se reflejarán en tu cotización.'
          );
        }

        if (
          devolutionCitySelectedNow?.city_name == this.devolutionCitySelected
        ) {
        ...
        this.vehicleForm
          .get('id_devolution_city')
          ?.setValue(devolutionCitySelectedNow?.city_id);
        this.devolutionCitySelected =
          devolutionCitySelectedNow?.city_name || 'Error';
        this.popoverDevolutionCity.hide();
  ...
  changeImage(type: string, option: any) {
    if (type === 'gamma') {
      ...
      this.gammaSelected.average_price =
        this.getAveragePriceForGamma(getGammaSelectedNow?.id || '') || 0;
  ...
  updateOverlayWidth() {
    if (this.vehicleTypeContainer) {
      const containerWidth =
        this.vehicleTypeContainer.nativeElement.offsetWidth;
  ...
  getGammas() {
    this.gammaService.getGammas().subscribe({
      next: (gammas) => {
        this.gammas = gammas.sort(
          (a, b) => a.precio_promedio - b.precio_promedio
        );
  ...
  getAveragePriceForGamma(gammaId: string) {
    const gammaPrice = this.averagePricesForSelect.find(
      (price) => price.gammaId === gammaId
    );
    ...
    if (this.vehicleForm.valid) {
      this.communicationService.dataForResume.emit({
        ...
        entry: {
          ...
          dateTime:
            this.entryDateSelected +
            ' ' +
            this.vehicleForm.get('entry_time')?.value,
          receive_at_airport: this.vehicleForm.get('receive_at_airport')?.value,
        },
        devolution: {
          city: this.devolutionCitySelected,
          dateTime:
            this.devolutionDateSelected +
            ' ' +
            this.vehicleForm.get('devolution_time')?.value,
      ...
      this.communicationService.message.emit(
        'Faltan datos para poder continuar'
      );
    }
  }
}
